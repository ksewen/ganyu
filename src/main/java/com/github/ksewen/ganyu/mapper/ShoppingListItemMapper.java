...
public interface ShoppingListItemMapper
    ...
    @Query("select new com.github.ksewen.ganyu.model.ShoppingListItemQueryModel"
    ...
    @Query(value = "select new com.github.ksewen.ganyu.model.ShoppingListItemQueryModel"
            + "(sli.id, sli.shoppingListId, sli.planToBuyId, ptb.name, ptb.brand, ptb.description, ptb.imageUrl, sli.bought, sli.createTime, sli.modifyTime) "
            + "from ShoppingListItem sli left join PlanToBuy ptb on sli.planToBuyId = ptb.id where sli.deleted = false and sli.shoppingListId = :shoppingListId", countQuery = "select count(shoppingListId) from ShoppingListItem where deleted = false and shoppingListId = :shoppingListId")
    Page<ShoppingListItemQueryModel> findAllByShoppingListIdJoinWithPlanToBuy(long shoppingListId, Pageable pageable);
}
