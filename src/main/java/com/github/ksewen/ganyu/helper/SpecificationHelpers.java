...
public class SpecificationHelpers {
  ...
  public void buildTimeRangeCondition(
      List<Predicate> list, DataSearchModel model, Root root, CriteriaBuilder criteriaBuilder) {
  ...
  public void buildCreateTimeRangeCondition(
      List<Predicate> list, DataSearchModel model, Root root, CriteriaBuilder criteriaBuilder) {
    if (model.getCreateTimeAfter() != null
        && model.getCreateTimeBefore() != null
        && model.getCreateTimeAfter().isBefore(model.getCreateTimeBefore())) {
      list.add(
          criteriaBuilder.between(
              root.get("createTime"), model.getCreateTimeAfter(), model.getCreateTimeBefore()));
    }
  }

  public void buildModifyTimeRangeCondition(
      List<Predicate> list, DataSearchModel model, Root root, CriteriaBuilder criteriaBuilder) {
    if (model.getModifyTimeAfter() != null
        && model.getModifyTimeBefore() != null
        && model.getModifyTimeAfter().isBefore(model.getModifyTimeBefore())) {
      list.add(
          criteriaBuilder.between(
              root.get("modifyTime"), model.getModifyTimeAfter(), model.getModifyTimeBefore()));
    }
  }
}
