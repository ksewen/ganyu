...
public class ShoppingListController implements LoggingController {
  ...
  public Result<ShoppingListResponse> add(@Valid @RequestBody ShoppingListInsertRequest request) {
    ShoppingListInsertModel model =
        this.beanMapperHelpers.createAndCopyProperties(request, ShoppingListInsertModel.class);
    ...
    ShoppingListResponse response =
        this.beanMapperHelpers.createAndCopyProperties(save, ShoppingListResponse.class);
    return Result.success(response);
  }

  @Operation(summary = "list all the shopping list")
  @GetMapping("/list")
  public PageResult<List<ShoppingListResponse>> list(
      ...
      @RequestParam(required = false, defaultValue = ParameterConstants.DEFAULT_COUNT_VALUE)
          Integer count) {
    ShoppingListSearchModel model =
        ShoppingListSearchModel.builder()
  ...
  @GetMapping("/detail")
  public Result<ShoppingListResponse> detail(
      @RequestParam(required = false) @NotNull(message = "{shopping.list.id.null}") Long id) {
    ShoppingList record = this.shoppingListService.findById(id, this.authentication.getUserId());
    ShoppingListResponse response =
        this.beanMapperHelpers.createAndCopyProperties(record, ShoppingListResponse.class);
    return Result.success(response);
  }

  @Operation(summary = "list all the shopping list items of the specific shopping list")
  @GetMapping("/items")
  public PageResult<List<ShoppingListResponseItem>> listByShoppingList(
      @RequestParam(required = false) Long shoppingListId,
      @RequestParam(required = false, defaultValue = ParameterConstants.DEFAULT_INDEX_VALUE)
          Integer index,
      @RequestParam(required = false, defaultValue = ParameterConstants.DEFAULT_COUNT_VALUE)
          Integer count) {
    Page<ShoppingListItemQueryModel> page =
        this.shoppingListService.findAllItemsByShoppingListId(
            shoppingListId, this.authentication.getUserId(), index, count);
    return PageResult.success(
        page.getContent().stream()
            .map(
                x ->
                    this.beanMapperHelpers.createAndCopyProperties(
                        x, ShoppingListResponseItem.class))
            .collect(Collectors.toList()),
        page.getPageable().getPageNumber(),
        page.getContent().size(),
        page.getTotalElements());
  ...
  public Result<ShoppingListResponse> markFinished(
      @RequestParam(required = false) @NotNull(message = "{shopping.list.id.null}") Long id) {
    ShoppingList record =
        this.shoppingListService.markFinished(id, this.authentication.getUserId());
    ShoppingListResponse response =
        this.beanMapperHelpers.createAndCopyProperties(record, ShoppingListResponse.class);
    return Result.success(response);
  }
