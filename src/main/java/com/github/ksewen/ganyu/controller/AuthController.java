...
public class AuthController implements LoggingController {
  ...
  public Result<UserInfoResponse> register(@Valid @RequestBody UserRegisterRequest request) {
    UserRegisterModel userRegisterModel =
        this.beanMapperHelpers.createAndCopyProperties(request, UserRegisterModel.class);
    User user =
        this.authService.register(userRegisterModel, AuthenticationConstants.USER_ROLE_NAME);
  ...
  public Result<JwtTokenResponse> tokenRefresh(
      @RequestParam @NotBlank(message = "{auth.refresh.token.null}") String refreshToken) {
