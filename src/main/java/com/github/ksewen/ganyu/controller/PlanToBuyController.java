...
public class PlanToBuyController implements LoggingController {
  ...
  public Result<PlanToBuyResponse> add(@Valid @RequestBody PlanToBuyInsertRequest request) {
    PlanToBuyInsertModel model =
        this.beanMapperHelpers.createAndCopyProperties(request, PlanToBuyInsertModel.class);
    model.setUserId(this.authentication.getUserId());
    PlanToBuy save = this.planToBuyService.save(model);
    PlanToBuyResponse response =
        this.beanMapperHelpers.createAndCopyProperties(save, PlanToBuyResponse.class);
    if (StringUtils.hasLength(save.getBusinessType())) {
      response.setBusinessType(
          this.businessHelpers.stringCommaSeparatedToList(save.getBusinessType()));
    }
  ...
  public Result<PlanToBuyResponse> modify(@Valid @RequestBody PlanToBuyModifyRequest request) {
    PlanToBuyModifyModel model =
        this.beanMapperHelpers.createAndCopyProperties(request, PlanToBuyModifyModel.class);
    PlanToBuy modify = this.planToBuyService.modify(model, this.authentication.getUserId());
    PlanToBuyResponse response =
        this.beanMapperHelpers.createAndCopyProperties(modify, PlanToBuyResponse.class);
    if (StringUtils.hasLength(modify.getBusinessType())) {
      response.setBusinessType(
          this.businessHelpers.stringCommaSeparatedToList(modify.getBusinessType()));
    }
    return Result.success(response);
  }
  ...
  public Result<Boolean> delete(
      @RequestParam(required = false)
          ...
          @Min(message = "{plan.to.buy.id.minimal}", value = 1)
          Long id) {
  ...
  @GetMapping("/list")
  public PageResult<List<PlanToBuyResponse>> list(
      ...
      @RequestParam(required = false, defaultValue = ParameterConstants.DEFAULT_COUNT_VALUE)
          Integer count) {
    PlanToBuySearchModel model =
        PlanToBuySearchModel.builder()
            ...
            .modifyTimeAfter(modifyTimeAfter)
            .modifyTimeBefore(modifyTimeBefore)
            .build();
    ...
    return PageResult.success(
        page.getContent().stream()
            .map(
                x -> {
                  PlanToBuyResponse item =
                      this.beanMapperHelpers.createAndCopyProperties(x, PlanToBuyResponse.class);
                  if (StringUtils.hasLength(x.getBusinessType())) {
                    item.setBusinessType(
                        this.businessHelpers.stringCommaSeparatedToList(x.getBusinessType()));
                  }
                  return item;
                })
  ...
  @GetMapping("/detail")
  public Result<PlanToBuyResponse> detail(
      @RequestParam(required = false) @NotNull(message = "{plan.to.buy.id.null}") Long id) {
    PlanToBuy record = this.planToBuyService.findById(id, this.authentication.getUserId());
    PlanToBuyResponse response =
        this.beanMapperHelpers.createAndCopyProperties(record, PlanToBuyResponse.class);
    if (StringUtils.hasLength(record.getBusinessType())) {
      response.setBusinessType(
          this.businessHelpers.stringCommaSeparatedToList(record.getBusinessType()));
    }
    return Result.success(response);
  }
