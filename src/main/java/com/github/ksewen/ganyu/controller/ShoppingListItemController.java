...
public class ShoppingListItemController implements LoggingController {
  ...
  @PostMapping("/add")
  public Result<List<ShoppingListResponseItem>> add(
      @Valid @RequestBody ShoppingListItemInsertRequest request) {
    List<ShoppingListItem> items =
        this.shoppingListItemService.add(
            request.getItemIds(), this.authentication.getUserId(), request.getShoppingListId());
    List<ShoppingListResponseItem> response =
        items.stream()
            .map(
                i ->
                    this.beanMapperHelpers.createAndCopyProperties(
                        i, ShoppingListResponseItem.class))
  ...
  @GetMapping("/detail")
  public Result<ShoppingListResponseItem> detail(
      @RequestParam(required = false) @NotNull(message = "{shopping.list.item.id.null}") Long id) {
    ShoppingListItemQueryModel model =
        this.shoppingListItemService.findById(id, this.authentication.getUserId());
    ShoppingListResponseItem response =
        this.beanMapperHelpers.createAndCopyProperties(model, ShoppingListResponseItem.class);
    return Result.success(response);
  }

  @Operation(summary = "mark a item of the shopping list has bought")
  @PostMapping("/mark-bought")
  public Result<ShoppingListResponseItem> markBought(
      @RequestParam(required = false) @NotNull(message = "{plan.to.buy.id.null}") Long id) {
    ShoppingListItemQueryModel record =
        this.shoppingListItemService.markBought(id, this.authentication.getUserId());
    ShoppingListResponseItem response =
        this.beanMapperHelpers.createAndCopyProperties(record, ShoppingListResponseItem.class);
    return Result.success(response);
  }
}
