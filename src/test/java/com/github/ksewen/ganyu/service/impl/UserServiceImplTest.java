...
class UserServiceImplTest {
  ...
  void addSuccess() {
    ...
    when(this.userMapper.saveAndFlush(
            argThat(
                user ->
                    this.name.equals(user.getUsername())
                        && this.name.equals(user.getNickname())
    ...
    assertThat(actual)
        .matches(u -> this.name.equals(u.getUsername()))
        .matches(u -> this.name.equals(u.getNickname()))
        .matches(u -> this.email.equals(u.getEmail()))
            ...
            argThat(
                list ->
                    !CollectionUtils.isEmpty(list)
                        ...
                        && Objects.equals(1L, list.get(0).getRoleId())
                        && Objects.equals(1L, list.get(1).getUserId())
                        && Objects.equals(2L, list.get(1).getRoleId())));
  }

  @Test
  void addWithExistUser() {
    when(this.userMapper.findOne(any(Specification.class)))
        .thenReturn(
            Optional.of(
                User.builder()
                    .id(1L)
    ...
    assertThat(exception)
        .matches(e -> ResultCode.ALREADY_EXIST.equals(e.getCode()))
        .matches(e -> ResultCode.ALREADY_EXIST.getMessage().equals(e.getMessage()));
  }

  @Test
  void modifyOwnSuccess() {
    when(this.userMapper.findById(anyLong()))
        .thenReturn(
            Optional.of(
                User.builder()
                    .id(1L)
    ...
    when(this.userMapper.saveAndFlush(
            argThat(
                user ->
                    Objects.equals(1L, user.getId())
                        && this.name.equals(user.getUsername())
                        && modifyNickname.equals(user.getNickname())
                        && this.email.equals(user.getEmail())
    ...
    assertThat(actual)
        .matches(u -> Objects.equals(1L, u.getId()))
        .matches(u -> this.name.equals(u.getUsername()))
        .matches(u -> this.modifyNickname.equals(u.getNickname()))
        .matches(u -> this.email.equals(u.getEmail()))
  ...
  void modifyAnotherSuccess() {
    when(this.roleService.findByUserId(anyLong()))
        .thenReturn(
            Arrays.asList(
                Role.builder().id(1L).name(AuthenticationConstants.ADMIN_ROLE_NAME).build()));
    when(this.userMapper.findById(anyLong()))
        .thenReturn(
            Optional.of(
                User.builder()
    ...
    UserModifyModel userModifyModel =
        UserModifyModel.builder().id(2L).nickname(this.modifyNickname).build();
    User actual = this.userService.modify(userModifyModel, 1L);
    assertThat(actual)
        .matches(u -> Objects.equals(2L, u.getId()))
        .matches(u -> this.name.equals(u.getUsername()))
        .matches(u -> this.modifyNickname.equals(u.getNickname()))
        .matches(u -> this.email.equals(u.getEmail()))
        .matches(u -> this.password.equals(u.getPassword()));
