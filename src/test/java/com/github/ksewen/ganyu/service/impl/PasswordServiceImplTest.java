...
class PasswordServiceImplTest {
  ...
  void modifySuccess() {
    ...
    when(this.passwordEncoder.matches(
            anyString(), argThat(encodedPassword -> this.password.equals(encodedPassword))))
            ...
            argThat(
                user ->
                    this.name.equals(user.getUsername())
                        ...
                        && modifyEncodedPassword.equals(user.getPassword()))))
        .thenReturn(
            User.builder()
                .id(1L)
                .username(this.name)
                .nickname(this.name)
                .email(this.email)
                .password(modifyEncodedPassword)
                .build());
  ...
  void resetSuccess() {
    ...
    verify(this.userService, times(1))
        .saveAndFlush(
            argThat(
                u ->
                    Objects.equals(1L, u.getId())
                        && this.name.equals(u.getUsername())
                        && this.name.equals(u.getNickname())
                        && this.email.equals(u.getEmail())
                        && this.modifyEncodedPassword.equals(u.getPassword())));
  }
