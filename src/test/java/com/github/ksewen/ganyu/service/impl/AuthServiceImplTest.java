...
class AuthServiceImplTest {
  ...
  void registerWhenSuccess() {
    ...
    when(this.userService.add(
            argThat(
                userRegisterModel ->
                    name.equals(userRegisterModel.getUsername())
                        && name.equals(userRegisterModel.getNickname())
                        && email.equals(userRegisterModel.getEmail())
                        && mobile.equals(userRegisterModel.getMobile())
                        && avatarUrl.equals(userRegisterModel.getAvatarUrl())
                        && StringUtils.hasLength(userRegisterModel.getPassword())),
            argThat(
                roles1 ->
                    !CollectionUtils.isEmpty(roles1)
                        && roles1.size() == 1
    ...
    assertThat(actual)
        .matches(u -> name.equals(u.getUsername()))
        .matches(u -> name.equals(u.getUsername()))
        .matches(u -> email.equals(u.getEmail()))
        .matches(u -> mobile.equals(u.getMobile()))
        .matches(u -> avatarUrl.equals(u.getAvatarUrl()))
        .matches(u -> encodePassword.equals(u.getPassword()));
  }
  ...
  void registerWhenInvalidRole() {
    ...
    UserRegisterModel registerModel =
        UserRegisterModel.builder().username(name).email(email).password(password).build();
    CommonException exception =
        Assertions.assertThrows(
            CommonException.class, () -> this.authService.register(registerModel, "NOT_EXIST"));
  ...
  void login() {
    ...
    when(this.authenticationManager.authenticate(
            argThat(
                token ->
                    name.equals(token.getPrincipal()) && password.equals(token.getCredentials()))))
        .thenReturn(new TestingAuthenticationToken(name, password));
    when(this.userDetailsService.loadUserByUsername(name))
        .thenReturn(
            JwtUserModel.builder().id(1L).username(name).nickname(name).password(password).build());
    ArgumentMatcher<UserDetails> matcher =
        argument -> name.equals(argument.getUsername()) && password.equals(argument.getPassword());
    ...
    assertThat(actual)
        ...
        .matches(t -> tokenModel.getToken().equals(t.getToken()))
