...
@SpringBootTest(
    classes = {
      GanyuApplicationTests.class,
      JacksonAutoConfiguration.class,
      JacksonHelpers.class,
      BeanMapperHelpers.class,
...
class AuthControllerTest {
  ...
  void register() throws Exception {
    when(this.authService.register(any(UserRegisterModel.class), anyString()))
        .thenAnswer(
            (Answer<User>)
                invocationOnMock -> {
                  ...
                  for (Object argument : invocationOnMock.getArguments()) {
                    if (argument instanceof UserRegisterModel) {
                      ...
                      mockUser.setNickname(
                          StringUtils.hasLength(model.getNickname())
                              ? model.getNickname()
                              : model.getUsername());
    ...
    this.mockMvc
        .perform(
            post("/auth/register")
                .content(this.jacksonHelpers.toJsonString(registerRequest))
        ...
        .andExpect(jsonPath("$.data.username").value("ksewen"))
